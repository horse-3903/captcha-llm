...
class Captcha:
    def __init__(self, width, high, ls=None, lc=4, fs=None, folder=os.path.join(wd, 'samples'), debug=False):
        
        if fs is None:
            fs = ['FONT_HERSHEY_COMPLEX', 'FONT_HERSHEY_SIMPLEX', 'FONT_ITALIC']
    ...
    def _tilt_img(self, img):
        tmp_img = img.copy()
        tmp_img.fill(255)
        tile_angle = np.random.randint(int(100*-math.pi/6), int(100*math.pi/6)) / 100
    ...
    def _shake_img(self, img, outer_top_left, outer_bottom_right, inner_top_left, inner_bottom_right):
    ...
    def _draw_basic(self, img, text):
        ...
        for index, letter in enumerate(text):
            ...
            font_color = tuple(int(np.random.choice(range(0, 156)))
                               for _ in range(3))
            cv2.putText(tmp_img, letter, bottom_left_coordinate, font_face, font_scale, font_color, font_thickness)
    ...
    def _draw_line(self, img):
        ...
        line_color = tuple(int(np.random.choice(range(0, 156))) for _ in range(3))
    ...
    def _put_noise(self, img):
        for i in range(600):
            ...
            dot_color = tuple(int(np.random.choice(range(0, 156))) for _ in range(3))
            img[y, x, :] = dot_color
...
if __name__ == '__main__':
    letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'K', 'M', 'N', 'P', 'R', 'T', 'U', 'V', 'W', 'X', 'Y']
    c = Captcha(120, 36, letters, folder="./results/new/part-4")
    c.batch_create_img(100)