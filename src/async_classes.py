...
class AsyncCaptchaTester:
    ...
    async def crosscheck_captcha(self, prompt_id: int, no_samples: int, batch_size: int = 10):
        ...
        for data_file in os.listdir(self.data_path):
            ...
            async with aiofiles.open(data_file_name, mode='r') as f:
        ...
        tasks = []
        for model in self.model_lst:
            for actual_solution, predicted_solution in data_lst:
                image_file_name = f"./data/{actual_solution}.png"
                
                if not os.path.exists(image_file_name):
                    continue
                
                modified_prompt = f"{prompt}\nThe solution provided earlier was '{predicted_solution}', but I need you to carefully verify this by analyzing the image again.\nEnsure that the text matches exactly, including case sensitivity and any possible distortions. If the previous solution is incorrect, provide the corrected text based solely on the image."
